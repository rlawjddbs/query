...
begin
	if grade in('특별', '일반') then
		if grade = '특별' then
		dbms_output.put_line(grade||'고객님 구매하신 상품 '||item_name||'이고 정상가 '||price||'원이며 할인가 '||(price-price*0.5)||'원 입니다.');
		else 
		dbms_output.put_line(grade||'고객님 구매하신 상품 '||item_name||'이고 정상가 '||price||'원이며 할인가 '||(price-price*0.3)||'원 입니다.');
		end if;
	end if;
end;
/
