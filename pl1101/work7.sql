...
begin
	...
	dbms_output.put_line('** ERROR : 혈액형을 대문자로 입력해 주세요.');
...
--   택시, 버스, 지하철이 아니라면  '대중교통이 아닙니다.' 를 출력하세요.
	if traffic= '택시' or traffic = '버스' or traffic = '지하철' then
		if traffic= '택시' then fee := 3800;
		elsif traffic= '버스' then fee := 1200;
		else fee := 1250;
		t_ment1 := '출퇴근에 사용하는 교통수단은 '||traffic||'이고 기본 요금: '||to_char(fee,'9,999,999')||' 원입니다.';
		t_ment2 := '왕복요금은 '||to_char((fee*2),'9,999,999')||'원이고, 한달 20일 기준 교통비는 '||to_char((fee*40),'9,999,999')||'원 입니다.';
