...
from		(
			...
			from		(
						...
						from		(
									select		e.empno, e.ename, to_char(e.hiredate, 'mm-dd-yyyy day') as hiredate, e.sal, d.dname, d.deptno, d.loc, z.zipcode, z.sido, z.gugun, z.dong, nvl(z.bunji, '번지없음') as bunji
									from		emp e, dept d, zipcode z
...
from			car_country cc, car_maker cma, car_model cmo
where			((cc.maker = cma.maker(+)) and (cma.model = cmo.model)) and cma.maker in ('현대', '기아', '삼성', 'BMW', 'AUDI') and (cmo.car_option like '%TCS%' and cmo.car_option like '%ABS%')
order by 		cmo.car_year desc, cmo.price desc;
...
select		e.empno, lower(e.ename) as ename, e.hiredate, e.sal, e.sal*0.033 as tax, to_char((e.sal+e.sal/12+nvl(e.comm, 0))-(e.sal*0.033),'9,999') as 실수령액,
...
select		e.empno, lower(e.ename) as ename, e.hiredate, e.sal, e.sal*0.033 as tax, trunc((e.sal+e.sal/12+nvl(e.comm, 0))-(e.sal*0.033)) as 실수령액,
